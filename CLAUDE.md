...
- **Мероприятия — это турниры.** `TrainingSession`/`TrainingBooking` описаны в
  схеме, но API их пока не обслуживает: расписание занятий — отдельный заход.
  Форма ответа `BookingEntry` под это уже рассчитана.
