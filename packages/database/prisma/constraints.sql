...
-- Телефон клуба — в том же формате, что у человека: «+7» и десять цифр.
--
-- Номер показывается посетителю ссылкой `tel:`, и «8 (391) 200-00-00» из неё
-- дозванивается не везде. Формат один на платформу, иначе клубы разойдутся в
-- написании и ссылка перестанет работать у половины.
ALTER TABLE "Tenant"
  ADD CONSTRAINT "Tenant_phone_format"
  CHECK ("phone" IS NULL OR "phone" ~ '^\+7[0-9]{10}$');

-- Почта клуба — с собакой и точкой в домене. Проверка нарочно грубая: точный
-- разбор адреса по RFC живёт в форме, а база стережёт только явный мусор.
ALTER TABLE "Tenant"
...
-- Чего здесь НЕТ и почему.
--
-- Возраст — «ребёнку меньше 14», «родителю 18» — в CHECK не выражается: это
...
-- проверяются в момент каждого действия: в день четырнадцатилетия строка
...
-- ---------------------------------------------------------------------------
-- 27. Страница клуба: ценности, соцсети, адрес зала, скрытые тренеры
-- ---------------------------------------------------------------------------
--
-- Накатано миграцией *_club_page_tabs (решения владельца от 25.09.2026).
