 ...
 *
 * До начала — да, после — нет, и это касается всех: и клиента, и
 * администратора. После начала запись не отменяется, а отмечается —
 * присутствием или неявкой. Без этой границы клиент, пропустивший занятие,
 * отменял его задним числом и платил позднюю отмену по ступеням политики
 * вместо неявки: 50% вместо 100%.
 *
 * Момент начала — граница включительно: в 19:00 занятие с началом в 19:00 уже
 * идёт. Та же граница, что у записи на мероприятие.
 */
export function cancellationOpen(startsAt: Date, now: Date): boolean {
