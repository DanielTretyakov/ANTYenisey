...
export interface SummaryEntry {
  ...
  charged: number;
  /**
   * Визит абонемента по этой записи сгорел.
   *
   * Отдельным признаком, а не деньгами: за поздней отменой по абонементу
   * денег нет вовсе — человек расплатился визитом при покупке, — но для
   * администратора это ровно та же поздняя отмена (решение владельца от
   * 20.09.2026). Без признака такие отмены не попадали в счётчик и человек с
   * абонементом выглядел аккуратнее, чем он есть.
   */
  burnedVisit?: boolean;
}
  ...
  for (const entry of entries) {
    ...
    switch (entry.status) {
      ...
      case 'CANCELLED':
        ...
        // Поздняя — та, за которую списали: деньгами по политике клуба или
        // сгоревшим визитом по абонементу. Считать её по времени отмены
        // значило бы завести второй расчёт рядом с тем, который уже
        // зафиксирован в записи.
        if (entry.charged > 0 || entry.burnedVisit) summary.lateCancellations += 1;
        break;
