...
export function templateViolations(slot: ClosureSlot): string[] {
  ...
  // Занятие — то же самое, что турнир: у него конкретная дата, и в
  // повторяющемся шаблоне ей неоткуда взяться. В шаблоне остаётся тип
  // тренировки, который дату не несёт.
  if ((slot.trainingSessionId ?? null) !== null) {
    violations.push(
      `Окно ${when}: в шаблон недели попадает тип тренировки, а не конкретное занятие`,
    );
  }
...
export function slotViolations(slot: ClosureSlot): string[] {
  ...
  // Занятие, в отличие от типа, необязательно: индивидуальное занятие закрывает
  // стол, но записываться на него некому. А вот у аренды или турнира ссылка на
  // занятие означала бы, что расписание врёт о том, чем стол занят.
  if (slot.purpose !== 'TRAINING' && (slot.trainingSessionId ?? null) !== null) {
    violations.push(`Окно ${when}: занятие указывается только у тренировки`);
  }
