 ...
 * детям младше 14. Исходящие заявки родителю по той же причине не
 * показываются: их отсутствие рассказало бы то же самое.
 */
const REQUEST_NOTICE: FamilyNotice = {
  message:
    'Если у этой почты есть учётка и её владельцу нет 14, он увидит заявку у себя в кабинете. ' +
...
export class FamilyService {
   ...
   * родителя — человек младше 14, который не может ни записаться сам, ни быть
   * записанным.
   *
  ...
  async requestForChild(guardianId: string, childId: string, desk: DeskContext): Promise<void> {
    ...
    if (!isChild(child.birthDate, today)) {
      throw new BadRequestException('С 14 лет человек записывается сам — закреплять его не нужно');
    }
   ...
   * при поиске у стойки. Опека, права по которой кончились (ребёнку 14),
