 ...
 * раздел 19), поэтому 14 и 18 живут только здесь — и проверяются в момент
 * действия, без джобы: в день четырнадцатилетия права родителя пропадают сами.
 */
 ...
 *
 * Только у действующей опеки и только пока ребёнку нет 14. Строка при этом не
...
export function decideCreateChild(input: {
  ...
  if (!isChild(input.childBirthDate, input.today)) {
    return {
      ok: false,
      status: 400,
      message: 'С 14 лет человек записывается сам — ему нужна обычная регистрация, а не учётка ребёнка',
    };
  }
...
export function decideAnswer(input: {
  ...
  if (!isChild(input.childBirthDate, input.today)) {
    return { ok: false, status: 409, message: 'С 14 лет закреплять вас за родителем уже не нужно' };
  }
...
/**
 * Что человек делает: меняет (запись, отмена, правка профиля) или смотрит
 * (свои записи, свой профиль).
 */
export type ActingMode = 'write' | 'read';

/**
 * За кого выполняется клиентское действие.
 *
 * Без `forId` человек действует сам. Менять что-то до 14 ему закрыто: иначе
 * «родитель руководит» было бы только словами, а ребёнок с учёткой
 * записывался бы сам. Смотреть своё — открыто: ребёнок отслеживает, куда его
 * записали. С `forId` — только за своего ребёнка младше 14 при действующей
 * опеке, и смотреть, и менять.
 *
...
export function decideActing(input: {
  ...
  if (input.forId === null || input.forId === input.callerId) {
    if (input.mode === 'write' && isChild(input.callerBirthDate, input.today)) {
      return {
        ok: false,
        status: 403,
        message: 'До 14 лет записывает родитель — или администратор клуба у стойки',
      };
    }
  ...
  if (!allowed) {
    return {
      ok: false,
      status: 403,
      message: 'За другого человека записывает только его родитель, пока ребёнку нет 14',
    };
  }
