...
export function ClubMark({
  ...
  // Логотип, в отличие от монограммы, в квадрат не запирается: клубный локап
  // обычно горизонтальный (знак слева, название справа), и вписанный в квадрат
  // по ширине он ужимается по высоте до нечитаемой полоски. Высота задана,
  // ширина свободна до потолка втрое шире — этого хватает и локапу, и круглой
  // эмблеме, а строку списка не разносит.
  const logoBox = { sm: 'h-9 max-w-27', md: 'h-12 max-w-36', lg: 'h-16 max-w-48' }[size];
  ...
  if (club.logoUrl) {
    return (
      <img
        ...
        //
        // Светлая подложка — не оформление, а условие читаемости. Логотип
        // приходит ссылкой на произвольную картинку: клуб мог нарисовать его
        // тёмной краской по прозрачному фону, и на тёмной теме такой знак
        // проваливается в фон целиком. Инвертировать чужой логотип фильтром
        // нельзя — у него могут быть свои цвета, и инверсия их переврёт.
        // Поэтому фон у знака всегда светлый, в обеих темах.
        className={cn(
          'w-auto shrink-0 rounded-control border border-border bg-ink-0 object-contain p-1',
          logoBox,
          className,
        )}
      />
